...
- Toda infraestrutura de nuvem é descrita como código (versionada), não criada manualmente no console — inclui as tabelas do **DynamoDB** (ver `04-dados-e-persistencia.md`).
...
- **Menor recurso possível sob demanda:** preferir capacidade on-demand/serverless e dimensionamento mínimo (ex.: DynamoDB em pay-per-request).
...
- **LocalStack** emula os serviços AWS na máquina, **incluindo o DynamoDB** (ver `04-dados-e-persistencia.md`).
- Ideal: um `docker-compose` que sobe LocalStack + aplicação, permitindo rodar o sistema completo offline.
